...
* If there is an actual project idea, but it's not ready to be planned
  ...
  * You can create an issue in the ``rse-projects`` Gitlab tracker to
    record the lead (state=`S::0-lead`), if you think this is concrete
    enough we should have a record of it.
  * Tell them they need to contact us when they are ready.
  * This serves as a insight for future needs, but we don't actively
    work yet.
  * If it's small enough that it would be driven by them coming and
    asking for help when they need it, suggest they use garage support
    instead.
...
* Two RSEs (one of which should be experienced in project planning) meet with
