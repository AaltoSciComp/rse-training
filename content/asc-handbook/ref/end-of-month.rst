End of month checklist
======================

* Halli

  * If you need to report worktime in Halli, do so and submit it.

* RSE scheduling spreadsheet (Google drive search: "RSE time scheduling")

  * Review and update the time allocation for the past month and ensure
    every :ref:`project according to the definition <what-is-project>`
    is listed, if you have done it during the month.

  * Each project entry is tagged ``#NNN`` corresponding to the
    rse-projects Gitlab repository.  (Not every row has to be a
    project there).

  * Un-bold the cell once it is updated :inote:`[bolding] someone may
    bold them on the last day of the month to indicate "not yet updated"`.

* rse-projects issue tracker (:ref:`ref <rse-timetracking>`):

  * Add any missing projects and give any updates.  You do not have to
    give updates on everything you do.

  * Update labels if needed. (for example ``Status:*``, ``Funding:*``)

* Events

  * If you have been to an event representing ASC (outreach, group
    meeting, workshop, discussion, teaching), add to events
    spreadsheet.
